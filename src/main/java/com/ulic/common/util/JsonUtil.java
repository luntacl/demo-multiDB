...
import com.ulic.core.exception.ExceptionCode;
import com.ulic.core.exception.PlatformIllegalArgumentException;
import com.ulic.core.exception.PlatformRuntimeException;
...
//        JSONArray source = sourceJsonObj.getJSONArray(arrayPath);
	...
	public static void validate(String jsonschema, String jsonString)
	{
		...
		try {
			if(StringUtils.isNotEmpty(jsonschema))
			{
			    ...
			    if(!report.isSuccess())
			    {
			    	...
			    	throw new PlatformRuntimeException(ExceptionCode.SYSTEM_ILLEGAL_REQUEST_B);
			    }
			}
			else
			{
		...
		} catch (PlatformIllegalArgumentException e) {
			throw e;
		} catch (Exception e) {
			log.error("====jsonSchemaValidation Exception====",e);
	    	throw new PlatformIllegalArgumentException(ExceptionCode.SYSTEM_ILLEGAL_REQUEST_B);
		}
